...
app.post("/create", (req, res) => {
	...
	const regDate = currentDate.getFullYear()+'-'+('0'+(currentDate.getMonth()+1)).slice(-2)+'-'+('0'+currentDate.getDate()).slice(-2)+' '+('0'+currentDate.getHours()).slice(-2)+':'+('0'+currentDate.getMinutes()).slice(-2)+':'+('0'+currentDate.getSeconds()).slice(-2)
...
app.post("/update", async (req, res) => {
	...
	if(typeof requestBody["Language"] !== 'undefined')
		if((requestBody["Language"].length > 0)&&(requestBody["Language"].indexOf("@result") == -1))
			updateFields.push("Language='"+requestBody["Language"]+"'")
		else{
			requestOK = false
			errorDescription += "Either Language is null or has invalid value like @result. "
		}
	...
	if(typeof requestBody["EnglishProficiency"] !== 'undefined')
		if(requestBody["EnglishProficiency"]==null){
			...
			if(['Decent','Beginner','Intermediate','Advanced'].includes(requestBody["EnglishProficiency"]))
				updateFields.push("EnglishProficiency='"+requestBody["EnglishProficiency"]+"'")
			else{
				requestOK = false
				errorDescription += "EnglishProficiency not one of Beginner,Intermediate and Advanced. "
			}
	...
	if(typeof requestBody["OnboardingComplete"] !== 'undefined'){
		if(typeof requestBody["OnboardingComplete"] === "boolean"){
			if(typeof requestBody["OnboardingStep"] === 'undefined'){
				requestOK = false
				errorDescription += "OnboardingStep is required along with OnboardingComplete status."
			}
			else{
				updateFields.push("OnboardingComplete="+requestBody["OnboardingComplete"])
	...
	if(requestOK==false){
		...
		zcqlQuestions.then(questionQueryResult => {
			...
			//If there are more than one records active for a mobile number, return error that there are more than one User.
				...
				console.log("Updated User Record - "+JSON.stringify(questionQueryResult))
				res.status(200).json(responseJSON);
			}
		}).catch((err) => {
			console.log(err);
...
app.post("/search", (req, res) => {
	...
	//Update consent
	let query = "Select ROWID, IsActive from User where Mobile='"+mobile+"' and isActive=true";
	console.log("Query : "+query);
	//Execute Query
	let zcqlQuestions = zcql.executeZCQLQuery(query);
	var responseJSON = {};
...
app.post("/searchfield", (req, res) => {
	...
	//Update consent
	let query = "Select ROWID, "+fields.join(", ")+" from Users where Mobile='"+mobile+"' and IsActive=true";
	console.log("Query : "+query);
	//Execute Query
	let zcqlQuestions = zcql.executeZCQLQuery(query);
	var responseJSON = {};
	zcqlQuestions.then(questionQueryResult => {
		...
		//Else return success
			...
			for(var i=0; i<fields.length; i++){
				if(typeof questionQueryResult[i]['Users'][fields[i]] !== 'undefined'){
					if(questionQueryResult[i]['Users'][fields[i]] != null){
						responseJSON[fields[i]] = questionQueryResult[i]['Users'][fields[i]]
						responseJSON['OperationStatus']='SUCCESS'
					}
				}
				else
					responseJSON[fields[i]] = null
			}
...
app.post("/createuserdata", (req, res) => {
	...
	.then((validationResultString)=>{
		...
		if(validationResult['OperationStatus']=='SUCCESS'){
			...
			functions.execute("searchUserbyMobile",{args:requestBody})
			.then((userROWIDResultString)=>{
				...
				if(userROWIDResult['OperationStatus']=='SUCCESS'){
					...
					functions.execute("addUserData",{args:argument})
						...
						try{
							functions.execute("sendResponseToGlific",{
									args:{
										...
										resultJSON : JSON.stringify({apiresponse:userDataResult})
									}
								}
							)
						}
