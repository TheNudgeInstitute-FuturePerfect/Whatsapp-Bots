  ...
  if (typeof mobile === "undefined") {
    ...
    if (typeof segment === "undefined") {
      ...
      if (typeof questions === "undefined") {
        ...
        zcql
          .executeZCQLQuery(
            "Select UserData.CREATEDTIME, UserData.Question, UserData.Answer from UserData left join Users on UserData.UserROWID = Users.ROWID where Segment = '" +
              segment +
              "' and Users.Mobile = " +
              mobile +
              " order by UserData.CREATEDTIME DESC"
          )
          .then((userdata) => {
            if (typeof userdata === "undefined") {
              result["OperationStatus"] = "NO_DATA";
              result["StatusDescription"] =
                "No question-answer record for the given segment and user";
              console.log("End of Execution: ", result);
              res.status(200).json(result);
              ...
              let questionRecord = userdata.filter((data) =>
                questions.includes(data.UserData.Question)
              );
              if (questionRecord.length == 0) {
                result["OperationStatus"] = "NO_DATA";
                result["StatusDescription"] =
                  "No question-answer record for the given questions for the user";
                console.log("End of Execution: ", result);
                res.status(200).json(result);
              } else {
                questionRecord = questionRecord.sort((a, b) => {
                  return a.UserData.CREATEDTIME >= b.UserData.CREATEDTIME
                    ? -1
                    : 1;
                });
                ...
                for (var i = 0; i < questions.length; i++) {
                  for (var j = 0; j < questionRecord.length; j++) {
                    if (
                      questions[i] == questionRecord[j]["UserData"]["Question"]
                    ) {
                ...
                result["AvgWordsPerText"] = Math.ceil(
                  result["TotalWords"] / result["TotalTexts"]
                );
                result["EnglishProficiency"] = null;
                const criteria = JSON.parse(
                  process.env.EnglishProficiencyCriteria
                );
                for (var i = 0; i < criteria.length; i++) {
                  if (
                    result["AvgWordsPerText"] >= criteria[i]["MinWords"] &&
                    result["AvgWordsPerText"] <= criteria[i]["MaxWords"]
                  ) {
                    result["EnglishProficiency"] =
                      criteria[i]["EnglishProficiency"];
                    break;
                  }
                }
                console.log("End of Execution: ", result);
