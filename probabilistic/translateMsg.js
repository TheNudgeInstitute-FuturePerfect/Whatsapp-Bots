			...
			if(typeof source === 'undefined'){
				...
				if(typeof target === 'undefined'){
					...
					if(typeof messageType === 'undefined'){
						...
						try{
							const systemPrompt = await catalystApp.zcql().executeZCQLQuery("Select SystemPromptsROWID from Sessions where SessionID = '"+sessionID+"'")
							translationSystemPrompt = systemPrompt[0]['Sessions']['SystemPromptsROWID']
						}
						catch(e){
							const systemPrompt = await catalystApp.zcql().executeZCQLQuery("Select ROWID from SystemPrompts where IsActive = true and Name = 'Translation'")
							translationSystemPrompt = systemPrompt[0]['SystemPrompts']['ROWID']
						}
						...
						.then(async insertResponse=>{					
							try{
								...
								if(translations.translations[0].translatedText == text){
									...
									const newText = translations.translations.length == 1 ? translations.translations[0].translatedText : translations.translations.map(data=>data.translatedText)
									request = {
										parent: `projects/${projectId}/locations/${location}`,
										contents: [newText],
										mimeType: allConfig['mimeTypes'][messageType.toLowerCase()],
								...
								responseObject['Translation'] = translations.translations.length == 1 ? translations.translations[0].translatedText : translations.translations.map(data=>data.translatedText)
								responseObject['DetectedLanguage'] = (typeof allConfig['codesLanguage'][detectedLanguage] === 'undefined') ? detectedLanguage : allConfig['codesLanguage'][detectedLanguage]
...
app.post("/translateattempts", async (req, res) => {
	...
	if(typeof sessionID === 'undefined'){
		...
		zcql.executeZCQLQuery("Select count(ROWID) from Sessions where Reply is not null and SessionID = '"+sessionID+" - Translation'")
		.then((translated)=>{
			if((typeof translated !== 'undefined')&&(translated!=null)&&(parseInt(translated[0]['Sessions']['ROWID'])>=parseInt(process.env.MaxTranslations))){
				...
				responseObject['PendingTranslations'] = process.env.MaxTranslations - translated[0]['Sessions']['ROWID']
				console.log("End of Execution: ",responseObject)
				res.status(200).json(responseObject)
			}
			const endTimeStamp = new Date()
