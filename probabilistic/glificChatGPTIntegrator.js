...
app.post("/chatgpt", async (request, response) => {
  ...
  if (feedbackPromptFlag == true) {
    ...
    if (
      typeof messagePrompt["Values"]["feedback prompt temperature"] !==
      "undefined"
    )
      temperature = messagePrompt["Values"]["feedback prompt temperature"];
  } else if (sessionType != "Normal Flow") {
    if (
      typeof messagePrompt["Values"][sessionType.toLowerCase() + " model"] !==
      "undefined"
    )
      model = messagePrompt["Values"][sessionType.toLowerCase() + " model"];
    if (
      typeof messagePrompt["Values"][
        sessionType.toLowerCase() + " temperature"
      ] !== "undefined"
    )
      temperature =
        messagePrompt["Values"][sessionType.toLowerCase() + " temperature"];
  }
