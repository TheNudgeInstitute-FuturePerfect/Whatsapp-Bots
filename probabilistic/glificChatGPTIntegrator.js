...
const transcribeAudio = async (audioURL, logger) => {
    ...
    try {
      ...
      const audioDetails = JSON.parse(await convertSpeechToText({ responseAVURL: audioURL }));
...
const saveContentInGCS = async (fileData, fileName, contentType, logger) => {
    ...
    try {
      ...
      const gcsFile = JSON.parse(await storeAudioFileinGCS({
        contentType,
        fileData,
        fileName,
      }));
...
app.post("/chatgpt", async (request, response) => {
  ...
  if (request.path === "/chatgpt") {
    ...
    const sessionType =
      requestSessionIDTokens.length > 1 ? requestSessionIDTokens[1] : "Normal Flow";
...
for (let i = startIndex; i < maxRows; i += 300) {
    let query = "";
    if (sessionType === 'SentenceFeedback') {
        query = "select ROWID, Message, Reply, CREATEDTIME, SystemPrompts.Content " +
            "from Sessions " +
            "left join SystemPrompts on Sessions.SystemPromptsROWID = SystemPrompts.ROWID " +
            "where Sessions.ROWID='" + storedSessionRecord['ROWID'] + "'";
    } else {
        query = "select ROWID, Message, Reply, CREATEDTIME, SystemPrompts.Content " +
            "from Sessions " +
            "left join SystemPrompts on Sessions.SystemPromptsROWID = SystemPrompts.ROWID " +
    ...
    for (let j = 0; j < queryOutput.length; j++) {
        ...
        if (j === queryOutput.length - 1 && queryOutput.length >= maxlinesofchat && !['ObjectiveFeedback', 'SentenceFeedback', 'Hint'].includes(sessionType)) {
            content = content + "\n" + messagePrompt['Values']['terminationprompt'];
            operationStatus = "END_OF_CNVRSSN";
            isActive = false;
        }

        if (!isConfigMsg && !['ObjectiveFeedback', 'SentenceFeedback', 'Hint'].includes(sessionType) && !message.includes("I want to continue our last conversation")) {
        ...
        if (queryOutput[j]['Sessions']['Reply'] !== null && sessionType !== 'SentenceFeedback') {
            sessionRecords.push({
                "role": "assistant",
