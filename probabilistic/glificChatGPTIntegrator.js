...
app.post("/chatgpt", async (request, response) => {
  ...
  if (sessionType === "SentenceFeedback") {
    ...
    query =
      ...
      mobile +
      " and SessionID = '" +
      (["Serious Mode", "Voice Challenge", "AskAnyDoubt"].includes(sessionType)
        ? requestBody.sessionId
        : sessionId) +
      "'" +
  ...
  for (; j < queryOutput.length; j++) {
    ...
    if (queryOutput[j]["Sessions"]["MessageType"].startsWith("UserMessage")) {
      ...
      if (j === queryOutput.length - 1) totalUserMessages++;
        ...
        if (
          !queryOutput[j + 1]["Sessions"]["MessageType"].startsWith(
            "UserMessage - Retry"
          )
        )
          totalUserMessages++;
  ...
  if (typeof chatGPTResponse["data"]["usage"] !== "undefined") {
    completionTokens =
      typeof chatGPTResponse["data"]["usage"]["completion_tokens"] !==
      "undefined"
        ? chatGPTResponse["data"]["usage"]["completion_tokens"]
        : 0;
    promptTokens =
      typeof chatGPTResponse["data"]["usage"]["prompt_tokens"] !== "undefined"
