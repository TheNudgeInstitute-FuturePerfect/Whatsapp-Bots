...
app.post("/allocatetopic", (req, res) => {
	...
	*/
	...
	if(typeof persona === 'undefined')
		persona = null
	else if(persona.startsWith("@result"))
		persona = null
	//Validate request
	mobile = mobile.toString().slice(-10)
	
	//Get the SystemPrompt details for the topics
	let query = "select distinct SystemPrompts.ROWID, SystemPrompts.Sequence, SystemPrompts.SupportingText, SystemPrompts.SupportingImageURL, SystemPrompts.SupportingAVURL, SystemPrompts.ObjectiveMessage from SystemPrompts where SystemPrompts.Name = '"+topic+"' and SystemPrompts.IsActive = true"
	if(persona != null)
	...
	.then((systemPrompts)=>{
		if(!((systemPrompts!=null)&&(systemPrompts.length>0))){
			...
			responseObject['StatusDescription']="No system prompts active for "+topic
			...
			if(persona != null){
				//Prepare the data to be returned
					...
					responseObject['SupportingText']=systemPrompts[0]['SystemPrompts']['SupportingText']
					responseObject['SupportingTextFlag']= responseObject['SupportingText'] != null
					responseObject['SupportingImageURL']=systemPrompts[0]['SystemPrompts']['SupportingImageURL']
					responseObject['SupportingImageURLFlag']= responseObject['SupportingImageURL'] != null
					responseObject['SupportingAVURL']=systemPrompts[0]['SystemPrompts']['SupportingAVURL']
					responseObject['SupportingAVURLFlag']= responseObject['SupportingAVURL'] != null
					responseObject['SupportingFlag'] = responseObject['SupportingTextFlag'] || responseObject['SupportingImageURLFlag'] || responseObject['SupportingAVURLFlag']
					responseObject['ObjectiveMessage'] = systemPrompts[0]['SystemPrompts']['ObjectiveMessage']
					responseObject['ObjectiveMessageFlag'] = responseObject['ObjectiveMessage'] != null
				...
				const systemPromptROWIDs = systemPrompts.map(record => record.SystemPrompts.ROWID)
				
				//Get the list of all prompts of the that has been practised by user
				query = "select DISTINCT Sessions.SessionID, Sessions.SystemPromptsROWID "+
						"from Sessions "+
						"where Sessions.Mobile = "+mobile+" and SystemPromptsROWID in ("+systemPromptROWIDs.join(",")+") "+
				...
				.then((sessions)=>{
					...
					if(!((sessions!=null)&&(sessions.length>0))){
						...
						for(var i=0;i<sessions.length;i++){
							for(var j=0;j<sessionCounts.length;i++){
								if(sessionCounts[j]['id']==sessions[i]['Sessions']['SystemPromptsROWID']){
					...
					const systemPrompt = systemPrompts.filter(data=>data.SystemPrompts.ROWID == responseObject['TopicID'])
					responseObject['SupportingText']=systemPrompt[0]['SystemPrompts']['SupportingText']
					responseObject['SupportingTextFlag']= responseObject['SupportingText'] != null
					responseObject['SupportingImageURL']=systemPrompt[0]['SystemPrompts']['SupportingImageURL']
					responseObject['SupportingImageURLFlag']= responseObject['SupportingImageURL'] != null
					responseObject['SupportingAVURL']=systemPrompt[0]['SystemPrompts']['SupportingAVURL']
					responseObject['SupportingAVURLFlag']= responseObject['SupportingAVURL'] != null
					responseObject['SupportingFlag'] = responseObject['SupportingTextFlag'] || responseObject['SupportingImageURLFlag'] || responseObject['SupportingAVURLFlag']
				...
				.catch(err=>{
					...
					console.log("End of execution due to error:",responseObject,"\nError:",err)
					res.status(200).json(responseObject)
				})
			}
		}
...
app.post("/topicpersonas", (req, res) => {
	...
	.then(promptsResult => {
		...
		if(allPrompts['OperationStatus']=="SUCCESS"){
			allPrompts = allPrompts["Prompts"].filter(data=>data.Persona!=null)
			if((typeof allPrompts !== 'undefined') && (allPrompts!=null) && (allPrompts.length>0)){
