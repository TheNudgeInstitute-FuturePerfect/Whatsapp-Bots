...
module.exports = (cronDetails) => {
    ...
    endDt = endDt.toString() + (endDt == 11 ? 'th': endDtLastDigit == 1 ? 'st': endDtLastDigit == 2 ? 'nd' : endDtLastDigit == 3 ? 'rd' : 'th') +" "+
            ((currentDate.getMonth()+1) == 1 ? 'January' : 
            (currentDate.getMonth()+1) == 2 ? 'February' : 
    ...
    .then(async (users) =>	{
        ...
        if(users == null){
                ...
                return new Promise(async (resolve, reject)=>{
                    ...
                    request(options, async function (error, response) {
						...
						if (error){
							...
							{
								console.log("Error returned by HSM Message API: "+JSON.stringify(hsmMsgResponse.errors));
								console.log("Request Parameters: "+JSON.stringify(options));
								reject("GLFC_API_ERR")
							}
							else
            ...
            users.forEach(async (record,i)=>{
                ...
                if(record.Users.Tags.includes('Cohort-2') && record.Users.Tags.includes(currentDt)){
                    param.push("7")
                    cohort = 2
                }
                else if(record.Users.Tags.includes('Cohort-1') && record.Users.Tags.includes(currentDt)){
                    ...
                    if(pendingPractices.data.OperationStatus != 'SUCCESS')
                        console.log(i+':Failed to pending practices for '+record.Users.Mobile)
                    else{
                        cohort = 1
                ...
                if(cohort!=null){
                    ...
                    console.log(i+": Nudge to be sent to Cohort "+cohort+" User "+ record.Users.Mobile);
					for(var index = 0 ; index < 100; index++){
                        try{
                            const output = await invokeGlificAPI(messageID,record.Users.GlificID,param)
                            /* sendGlificHSMMsg({
                            ...
                            if(output=="SUCCESS"){
                                console.log(i+":Nudge sent to Cohort "+cohort+" User "+record.Users.Mobile)
                                try{
                                    let eventData = {
                                        SessionID: "Welcome Message",
                                        Event : "Cohort "+cohort+" Welcome Message Sent (HSM Message)",
                    ...
                    console.log(i+": Nudge not to be sent to "+ record.Users.Mobile+" with Tags = "+record.Users.Tags);
                }
            })
            .catch(err => {
